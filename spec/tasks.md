---
{
  ...
  "filetype": "document",
  "filename": "tasks",
  "created_at": "2026-06-17T01:07:41.367Z",
  "updated_at": "2026-06-17T01:07:43.251Z",
  "meta": {
    "location": "/",
    "tags": [],
    "categories": [],
...
**Contracts:** `CLAUDE.md` (build) · `spec/SKILLS.md` (verification) **Rule:** Each checkbox must be completed and smoke-tested before the next is started. **Test rule:** Every implementation checkbox that generates test files must be committed together with those files.
...
**Expected sysimage:** `C:\Users\johnx\AppData\Local\org.julialab.ide\julialab.dll`**Smoke test pass:** log line present AND elapsed time recorded
...
### Hotfix 002 — 2026-06-16

**File:** `app/internals/src/actors/communication_actor/connection.rs`**Bug:** Four LocalSocketStream::connect retry loops ran blocking Windows syscalls (CreateFile/WaitNamedPipe) inside tokio::task::spawn, starving the async worker pool during restart and deadlocking the kill/disconnect path — perceived OS-level freeze. **Fix:** All four loops moved to spawn_blocking with sync sleeps/locks; shared retry logic extracted to connect_with_retry~~. Discriminating regression test added (current_thread flavor, injected blocking connect fn). ~~**~~Tests:~~**~~ green / 2 ~~**~~Commits:~~** ~~\[002 hash\], \[002 amendment hash\]~~

### ~~Hotfix 002b — 2026-06-16~~

**~~File:~~** `app/internals/src/actors/communication_actor/mod.rs`**~~Bug:~~**~~ DisconnectFromPipes did not synchronously reset actor-level is_connected, to_julia_pipe, or from_julia_pipe. On restart, IsConnected returned stale true before the FROM pipe reconnected, causing premature ActivateProject and an infinite response wait. ~~**~~Fix:~~**~~ Three synchronous resets added to the handler body before the async disconnect task is spawned. Both copies of is_connected (actor-level and state-level Arc&lt;Mutex&gt;) verified cleared on disconnect. ~~**~~Tests:~~**~~ green / 2 (both communication_actor tests pass) ~~**~~Commits:~~** ~~\[002b fix hash\], \[002b test hash\]~~
...
- [ ] **~~Run:~~** `cd app/app && npm test` ~~— must show ~~**~~11 tests, 0 failures~~** ~~before proceeding to 3b (12 tests total once PlotsToolbar import test is added in Task 3b)~~
...
**~~Architecture decision:~~**~~ AppsPanel uses the ~~**~~center pane replacement~~** ~~pattern (same as LiveEditorPanel), not a floating overlay. When~~ `layoutStore.showAppsPanel` ~~is true, the editor area is replaced by AppsPanel. This keeps the splitpanes layout intact and is consistent with how LIVE EDITOR works.~~

- [ ] ~~Read~~ `app/app/src/components/layouts/MainLayout.vue`

- [ ] ~~In the center pane area, add:~~ `<AppsPanel v-else-if="layoutStore.showAppsPanel" />`~~alongside the existing~~ `<LiveEditorPanel>` ~~and~~ `<FileEditor>` ~~conditions~~

- [ ] `AppsPanel` ~~must include a close button that sets~~ `layoutStore.showAppsPanel = false`~~to restore the editor view~~
...
- [ ] **~~Generate~~** `tests/manual/T6_insert_postmessage.md` ~~(steps + 5 test cases from~~ `SKILLS.md`~~) This file must exist before the discovery steps below so you have somewhere to record findings.~~

- [ ] **~~Discover:~~**~~ Read Pluto WebSocket source at:~~ `~/.julia/packages/Pluto/<version>/src/webserver/`~~and find the correct~~ `type` ~~field name for cell insertion~~
